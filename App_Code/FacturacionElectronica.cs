...
namespace FacturacionElectronica
{
    ...
    public class Facturas
    {
        ...
        {
            ejecuta.baseDatos = "PVW";
            sql = "select top 1 IdCfd,IdTimbre,noCertificadoSat,fechaTimbrado,uuid,selloSat,selloCFD,rutaArchivo,cadenaOriginal,noCertificadoCfd,qr from Timbrado_f where IdCfd=" + idCfd.ToString() + " order by idtimbre desc";
            info = ejecuta.dataSet(sql);
        }
        ...
        public void actualizaEstatusFactura()
        {
            ...
            sql = "update EncCFD_f set EncEstatus='" + estatus + "',encfechacancel='" + fecha.ToString("yyyy-MM-dd") + "',enchoracancel='" + fecha.ToString("HH:mm:ss") + "' where IdCfd=" + idCfd.ToString() + " update facturas set estatus_factura='" + estatus + "'" + cambio + " where idcfd=" + idCfd.ToString();
            info = ejecuta.insertUpdateDelete(sql);
        }
    }
...
" isnull(e.certificado,'') as certificado,e.idcfd,e.tipo from EncCFD_f e where e.IdCfd=" + idCfd.ToString();
        ...
        public void actualizaTimbrado(string certificado, string fecha, string uuid, string selloSat, string selloCFD, byte[] qr, string ruta, string cadena, string certificadoCfd, string certificadoEmisor)
        {
            ...
            "update EncCFD_f set EncFolioUUID='{3}', EncSello='{5}',EncCertificado='{1}',EncTimbre='{4}', encestatus='T',certificado='{9}' where IdCfd={0}",
        ...
        {
            ejecuta.baseDatos = "PVW";
            sql = string.Format("update EncCFD_f set EncFechaGenera='{0}', EncHoraGenera='{1}' where IdCfd={2}", fechaGeneracion.ToString("yyyy-MM-dd"), fechaGeneracion.ToString("HH:mm:ss"), documento);
            info = ejecuta.insertUpdateDelete(sql);
        }
        ...
        public bool verificaDocumentoTimbrado(int documento)
        {
            bool timbrado = false;
            ejecuta.baseDatos = "PVW";
            sql = string.Format("select COUNT(*) from enccfd_f where idcfd={0} and encestatus='T'", idCfd);
        ...
        {
            ejecuta.baseDatos = "PVW";
            sql = string.Format("select top 1 t.uuid,t.rutaArchivo,e.EncEmRFC,em.idemisor,t.noCertificadoSat,t.fechatimbrado,t.sellosat,t.sellocfd,t.cadenaOriginal,t.nocertificadocfd from Timbrado t inner join EncCFD_f e on e.idcfd = t.idcfd inner join emisores em on em.emrfc=e.EncEmRFC where t.idcfd={0}", idCfd);
            info = ejecuta.dataSet(sql);
        }
        ...
        internal void actualizaTimbradoPrb(string ruta)
        {
            ...
            "update EncCFD_f set EncFolioUUID=(select top 1 uuid from timbrado where idcfd={0}), EncSello=(select top 1 selloCfd from timbrado where idcfd={0}),EncCertificado=(select top 1 noCertificadoSat from timbrado where idcfd={0}),EncTimbre=(select top 1 selloSat from timbrado where idcfd={0}), encestatus='T' where IdCfd={0}",
                idCfd, ruta);
