...
public partial class FacturacionGral : System.Web.UI.Page
{
    ...
    private void calculaIvaTras(GridDataItem item)
    {
        ...
        decimal val= Math.Truncate((subTotalCncpto + ivaTras)*100)/100;
        totalLinea = val;
        ((Label)item.FindControl("lblTotalCpto")).Text = (totalLinea).ToString();
    }
    ...
    private void calculaTotalDescuento()
    {
        ...
        foreach (GridDataItem fila in grdDocu.Items)
        {
            ...
            if (PctjeDsctoGlb != 0)
            {
                ...
                {
                    //////////////////////////////////////////////////////////////////
                    //decimal res= ((subTotalCncpto * PctjeDsctoGlb) / 100);
                    //decimal val = 0.00M;
                    //val = res % 1;
                    //if (val < 0.5M)
                    //    totDsctoGbl = totDsctoGbl + res;
                    //else if (val >= 0.5M)
                    //    val = Math.Round(res, 2);
                    decimal a = Math.Truncate(Convert.ToDecimal(((Label)fila.FindControl("lblSubTotal")).Text)*100)/100;


                    totDsctoGbl = totDsctoGbl+ Importe - a;

                    //totDsctoGbl = totDsctoGbl + ((subTotalCncpto * PctjeDsctoGlb) / 100);

                    decimal val = Math.Truncate(totDsctoGbl * 100) / 100;
                    totDsctoGbl = val;
        ...
        ((Label)fvwResumen.Row.FindControl("lblDsctoGlb")).Text = totDsctoGbl.ToString("N2");
        calculaSubTotNeto();
    }
    ...
    protected void txtPctjeDsctoGlb_TextChanged(object sender, EventArgs e)
    {
        ...
        foreach (GridDataItem item in grdDocu.Items)
        {
            ...
            if (PctjeDsctoGlb != 0 && subTotalCncpto != 0)
            {
                //decimal b=Math.Truncate(Convert.ToDecimal(((Label)item.FindControl("lblTotalCpto")).Text)*100)/100;
                //dtoGlobConcepto = ImporteCpto - b;
                //dtoGlobConcepto = ((subTotalCncpto * PctjeDsctoGlb) / 100);
                subTotalCncpto = subTotalCncpto - ((subTotalCncpto * PctjeDsctoGlb) / 100);
                tru = Math.Truncate(subTotalCncpto * 100) / 100;
                dtoGlobConcepto = ImporteCpto - tru;

            }


            string global = dtoGlobConcepto.ToString();
            string sub = tru.ToString();
            ...
            calculaIsrRet(item);
            tru = 0.00M;
            decimal x = Math.Truncate(Convert.ToDecimal(global) * 100) / 100;
            totDsctoGbl = totDsctoGbl + x;
        }
        if (PctjeDsctoGlb != 0)
        {
    ...
    private void calculaTotales()
    {
        ...
        foreach (GridDataItem fila in grdDocu.Items)
        {
            ...
            decimal val = Math.Truncate(totImpTras * 100) / 100;
            totImpTras = val;
        }
                ...
                foreach (GridDataItem fila in grdDocu.Items)
                {
                    ...
                    foreach (DataRow dato in dt.Rows)
                    {
                        if (filaIns == filasDt)
                        {
                            ...
                            {
                                dtoGlobConcepto = ((subTotalCncpto * PctjeDsctoGlb) / 100);
                                decimal val = Math.Truncate(dtoGlobConcepto * 100) / 100;
                                dtoGlobConcepto = val;
        ...
        else
        {
            ...
            using (SqlConnection conLoc = new SqlConnection(ConfigurationManager.ConnectionStrings["PVW"].ConnectionString))
            {
                try
                {
                    ...
                    {
                        ///////////////////////////////////////////////////////////////Corregir desde aqui!!!
                        decimal valUnit = Convert.ToDecimal(r[12]);
                        int cant = Convert.ToInt16(r[3]);
                        decimal impte = Convert.ToDecimal(r[12]);
                        decimal totDcto = Convert.ToDecimal(r[6]);
                        //decimal totDcto = (valUnit * (Convert.ToDecimal(r[6]) / 100)) * cant; //   Convert.ToDecimal(r[7]) * cant;
                        //decimal subTot = impte - totDcto; //Convert.ToDecimal(r[5]);
                        decimal subTot = 0.0M;
                        if (porcDctoGlob > 0)
                            //////////////////////////////////////////////////
                            //subTot = subTot - ((subTot * porcDctoGlob) / 100);
                            subTot = Convert.ToDecimal(r[14]);      //Math.Round(subTot - ((subTot * porcDctoGlob) / 100),3);

                        
                        decimal iva16 = Convert.ToDecimal(r[13]);   //Math.Round(subTot * 0.16M,2);
                        decimal Total = impte + iva16;             //Math.Round(subTot + iva16,2);
                        subTot = Total - iva16;
                        int clavesat;
